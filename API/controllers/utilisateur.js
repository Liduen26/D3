...
const getUtilisateurs = async (req, res) => {
  try {
    // Requete
    let cursor = client.db().collection("utilisateurs").find().sort({ noms: 1 });
...
// ----Partie permettant de modifier 1 ou plusieurs champs sans que les autres soient effacés----
    let cursor1 = client.db().collection("utilisateurs").find({ _id: id });
    let result1 = await cursor1.toArray();

    if(noms == null){
      noms = result1[0].noms;
    }
    if(adresse == null){
      adresse = result1[0].adresse;
    }
    if(telephone == null){
...
// ----------------------------------------------------------------------------------------------
    ...
    let result = await client.db().collection("utilisateurs").updateOne({ _id: id }, { $set: { noms, adresse, telephone } });
