...
function inMove(e, myWindow) {
    ...
    function mouseMove(e) {
        const newCo = { x: (e.x - start.x), y: (e.y - start.y) };

        // Application des coordonées
        myWindow.x.value += newCo.x;
        myWindow.y.value += newCo.y;
...
.default {
    ...
    resize: both;
    overflow: auto;
    transition: border-color .2s, left .02s, top .02s, height .02s, width .02s;
}
